...
class PlayState extends MusicBeatState
{
	var sans:FlxSprite;
	var ass:FlxSprite;
	var isfont:String = "vcr.ttf";

	private var isDownscroll:Bool = false;

	private var floathealth:Float = 0;

	public var isoldcamera:Bool = true;

	var deeznuts:FlxSprite;
	var deeznuts2:FlxSprite;

	var deeznuts3:FlxSprite;
	var deeznuts4:FlxSprite;

	var deeznuts5:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'deeznuts':
				GameOverSubstate.loopSoundName = 'gameover theme';
				GameOverSubstate.characterName = 'dead';

				defaultCamZoom = 0.7;
				curStage = 'deeznuts';
				deeznuts3 = new FlxSprite(-450, -200).loadGraphic(Paths.image('hmmm2'));
				deeznuts3.setGraphicSize(Std.int(deeznuts3.width * 2));
				deeznuts3.updateHitbox();
				deeznuts3.antialiasing = ClientPrefs.globalAntialiasing;
				deeznuts3.active = false;
				add(deeznuts3);

				deeznuts4 = new FlxSprite(-450, -200).loadGraphic(Paths.image('hmmm1'));
				deeznuts4.setGraphicSize(Std.int(deeznuts4.width * 2));
				deeznuts4.updateHitbox();
				deeznuts4.antialiasing = ClientPrefs.globalAntialiasing;
				deeznuts4.active = false;
				add(deeznuts4);

				deeznuts3.alpha = 0;
				deeznuts4.alpha = 0;

				deeznuts = new FlxSprite(-600, -200).loadGraphic(Paths.image('bg1'));
				deeznuts.setGraphicSize(Std.int(deeznuts.width * 2));
				deeznuts.updateHitbox();
				deeznuts.antialiasing = ClientPrefs.globalAntialiasing;
				deeznuts.active = false;
				add(deeznuts);

				deeznuts2 = new FlxSprite(-600, -200).loadGraphic(Paths.image('bg2'));
				deeznuts2.setGraphicSize(Std.int(deeznuts2.width * 2));
				deeznuts2.updateHitbox();
				deeznuts2.antialiasing = ClientPrefs.globalAntialiasing;
				deeznuts2.active = false;

				deeznuts5 = new FlxSprite(-600, -200).loadGraphic(Paths.image('bg3'));
				deeznuts5.setGraphicSize(Std.int(deeznuts5.width * 2));
				deeznuts5.updateHitbox();
				deeznuts5.antialiasing = ClientPrefs.globalAntialiasing;
				deeznuts5.active = false;
				add(deeznuts5);

				sans = new FlxSprite(100,500);
				sans.frames = Paths.getSparrowAtlas('sanssssssss','shared');
				sans.animation.addByPrefix('idle', 'sanssss0', 24);
				sans.animation.play('idle');
				sans.antialiasing = ClientPrefs.globalAntialiasing;
				sans.alpha = 0;
				add(sans);

				ass = new FlxSprite(100,600);
				ass.frames = Paths.getSparrowAtlas('asssssssssssssssssssss','shared');
				ass.animation.addByPrefix('idle', 'ass0', 24);
				ass.animation.play('idle');
				ass.antialiasing = ClientPrefs.globalAntialiasing;
				ass.alpha = 0;
				add(ass);

				preload2('HURTnoteSplashes');
				preload('knife','shared');
				preload('burh','shared');
		}
		...
		switch(curStage)
		{
			...
			case 'deeznuts':
				...
				var gfswagShader:ColorSwap = null;
				gfswagShader = new ColorSwap();
				gfswagShader.brightness = 0;
				gf.shader = gfswagShader.shader;
				gfswagShader.brightness -= 0.2;
		}
		...
		timeTxt.setFormat(Paths.font(isfont), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font(isfont), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font(isfont), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(curStage == 'deeznuts')
			{
				CoolUtil.precacheSound('miss');
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (curStage == "deeznuts"){
			deeznuts4.y += Math.sin(floathealth);
			deeznuts4.y += Math.cos(floathealth);
		}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(isDownscroll) doKill = daNote.y > FlxG.height;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(Paths.formatToSongPath(SONG.song) == 'devilovania')
						{
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		switch(daNote.noteType) {
			case 'CHARA Note':
				...
				if(dad.animation.getByName('fight') != null) {
					dad.playAnim('fight', true);
					dad.specialAnim = true;
				}
				health -= 0.3;
		}
			...
			if(daNote.noteType == 'CHARA Note')
				{
					...
					FlxG.sound.play(Paths.sound('attack'),0.6);
				}
			else
				{
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		if (!boyfriend.stunned)
		{
			...
			if(curStage == 'deeznuts')
				{
					FlxG.sound.play(Paths.sound('miss'), FlxG.random.float(0.1, 0.2));
				}
			else
				{
			...
			if(!note.noAnimation) {
				...
				//}else{
					if(note.gfNote) {
						...
						gf.holdTimer = 0;
					} else if (note.noteType == 'CHARA Note'){
						if(boyfriend.animation.getByName('dodge') != null) {
							boyfriend.playAnim('dodge', true);
							boyfriend.specialAnim = true;
						}
		...
		if(Paths.formatToSongPath(SONG.song) == 'devilovania')
			{
				switch(curStep)
				{
					...
					case 384:
						idk(false);
					case 400 | 444 | 574 | 860 | 1080 | 1120 | 1166 | 1196: //downScroll
						downscroll(true);
					case 414 | 506 | 638 | 890 | 1104 | 1150 | 1181 | 1212: //upScroll
						downscroll(false);
					case 398:
						FlxTween.tween(sans, {alpha: 1}, 0.5);
					case 505:
						FlxTween.tween(sans, {alpha: 0}, 0.5);
					case 767:
						FlxTween.tween(ass, {alpha: 1}, 0.5);
					case 889:
						FlxTween.tween(ass, {alpha: 0}, 0.5);
					case 784 | 797 | 812 | 827 | 848 | 858 | 874 | 1024 | 1035 | 1065 | 1088 | 1103 | 1119 | 1136 | 1152 | 1168 | 1184 | 1200:
						buster();
				}
				if(curStep >= 257 && curStep < 384)
					{
						knife();
					}
				if(curStep >= 506 && curStep < 636)
					{
						knife();
					}
				if(curStep >= 768 && curStep < 889)
					{
						knife();
					}
				if(curStep >= 1019 && curStep < 1226)
					{
						knife();
					}
			}
	...
	function changeDadCharacter(id:String)
		{
			var olddadx = dad.x;
			var olddady = dad.y;
			triggerEventNote('Change Character', '1', id);
			dad.setPosition(olddadx, olddady);
			trace('dad x position =' + ' ' + dad.x);
			trace('dad y position =' + ' ' + dad.y);
		}
	function changeGfCharacter(id:String)
		{
			var oldgfx = gf.x;
			var oldgfy = gf.y;
			triggerEventNote('Change Character', '2', id);
			gf.setPosition(oldgfx, oldgfy);
			trace('gf x position =' + ' ' + gf.x);
			trace('gf y position =' + ' ' + gf.y);
		}
	function changeBfCharacter(id:String)
		{
			var oldbfx = boyfriend.x;
			var oldbfy = boyfriend.y;
			triggerEventNote('Change Character', '0', id);
			boyfriend.setPosition(oldbfx, oldbfy);
			trace('boyfriend x position =' + ' ' + boyfriend.x);
			trace('boyfriend y position =' + ' ' + boyfriend.y);
		}
	...
	{
		var newthing:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image(graphic));
		newthing.visible = false;
		add(newthing);
		remove(newthing);
	}
	function idk(idk:Bool)
		{
			if(idk)
				{
					FlxG.sound.play(Paths.sound('hm'));
					FlxTween.tween(FlxG.camera, {zoom: 0.5}, 0.1, {onComplete:function (twn:FlxTween) {defaultCamZoom = 0.4;}});
					changeDadCharacter('chara pov');
					dad.setPosition(100 + 370 + 160,500 - 790);
					boyfriend.alpha = 0;
					gf.alpha = 0;
					deeznuts.alpha = 0;
					deeznuts2.alpha = 0;
				...
				{
					FlxTween.tween(FlxG.camera, {zoom: 0.7}, 0.1, {onComplete:function (twn:FlxTween) {defaultCamZoom = 0.7;}});
					changeDadCharacter('chara');
					dad.setPosition(100,500);
					boyfriend.alpha = 1;
					gf.alpha = 1;
					deeznuts.alpha = 1;
					deeznuts2.alpha = 1;
					...
					if(ClientPrefs.downScroll)
						{
							...
							for(note in playerStrums)
								{
									FlxTween.tween(note, {y: 50}, 0.1);
								}
							for(note in opponentStrums)
								{
									FlxTween.tween(note, {y: 50}, 0.1);
								}
							FlxTween.tween(timeTxt, {y: 19}, 0.1);
							FlxTween.tween(timeBarBG, {y: 19 + (timeTxt.height / 4)}, 0.1);
							FlxTween.tween(timeBar, {y: 19 + (timeTxt.height / 4) + 4}, 0.1);
							FlxTween.tween(healthBarBG, {y: FlxG.height * 0.89}, 0.1);
							FlxTween.tween(healthBar, {y: FlxG.height * 0.89 + 4}, 0.1);
							FlxTween.tween(iconP1, {y: FlxG.height * 0.89 + 4 - 75}, 0.1);
							FlxTween.tween(iconP2, {y: FlxG.height * 0.89 + 4 - 75}, 0.1);
							FlxTween.tween(botplayTxt, {y: 19 + (timeTxt.height / 4) + 55}, 0.1);
							FlxTween.tween(scoreTxt, {y: FlxG.height * 0.89 + 36}, 0.1);
						}
					else
						{
							isDownscroll = true;
							for(note in playerStrums)
								{
									FlxTween.tween(note, {y: FlxG.height - 150}, 0.1);
								}
							for(note in opponentStrums)
								{
									FlxTween.tween(note, {y: FlxG.height - 150}, 0.1);
								}
							FlxTween.tween(timeTxt, {y: FlxG.height - 44}, 0.1);
							FlxTween.tween(timeBarBG, {y: FlxG.height + (timeTxt.height / 4)}, 0.1);
							FlxTween.tween(timeBar, {y: FlxG.height + (timeTxt.height / 4) + 4}, 0.1);
							FlxTween.tween(healthBarBG, {y: 0.11 * FlxG.height}, 0.1);
							FlxTween.tween(healthBar, {y: 0.11 * FlxG.height + 4}, 0.1);
							FlxTween.tween(iconP1, {y: 0.11 * FlxG.height + 4 - 75}, 0.1);
							FlxTween.tween(iconP2, {y: 0.11 * FlxG.height + 4 - 75}, 0.1);
							FlxTween.tween(botplayTxt, {y: FlxG.height + (timeTxt.height / 4) - 78}, 0.1);
							FlxTween.tween(scoreTxt, {y: 0.11 * FlxG.height + 36}, 0.1);
						}
				}
			else
				{
	...
	function knife()
		{
			if (curStep % 2 == 0)
			{
				...
				var knife = new FlxSprite(2000).loadGraphic(Paths.image('knife'));
				knife.cameras = [camHUD];
				knife.antialiasing = ClientPrefs.globalAntialiasing;
				knife.alpha = 0.6;
				add(knife);
				var y = 0;
				if (curStep % 16 < 8)
				...
				{
					y = 800 - (100 * (curStep % 8) - 200); 
				}
				knife.y = y - 600;
				FlxTween.tween(knife, {x: -3000}, 4, {
					onComplete: function(twn:FlxTween)
					{
						remove(knife);
					}
	...
	function buster()
		{
			var buster:FlxSprite = new FlxSprite(-100);
			buster.frames = Paths.getSparrowAtlas('burh','shared');
			buster.animation.addByPrefix('buster', 'Symbol 1', 24,false);
			buster.animation.play('buster');
			buster.antialiasing = ClientPrefs.globalAntialiasing;
			buster.cameras = [camHUD];
			add(buster);
			buster.scale.x = 1.5;
			if(isDownscroll) 
				{
			...
			{
				FlxG.sound.play(Paths.sound('buster'), 0.7);
				var drain:Float = health;
				drain -= 0.3;
				if (drain <= 0){
					//...
				}
			...
			if(buster.animation.curAnim.finished)
				{
					remove(buster);
